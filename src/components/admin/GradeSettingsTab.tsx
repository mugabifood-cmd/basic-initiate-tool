...
export default function GradeSettingsTab() {
  ...
  const fetchBoundaries = async () => {
    ...
    const {
      data,
      error
    } = await supabase.from('grade_boundaries').select('*').order('min_score', {
      ascending: false
    });
    if (error) {
      toast({
  ...
  const checkOverlap = (minScore: number, maxScore: number, excludeId?: string) => {
    return boundaries.some(b => {
      ...
      return minScore >= b.min_score && minScore <= b.max_score || maxScore >= b.min_score && maxScore <= b.max_score || minScore <= b.min_score && maxScore >= b.max_score;
    });
  };
  const handleSave = async () => {
    ...
    if (editingBoundary) {
      const {
        error
      } = await supabase.from('grade_boundaries').update({
      ...
      const {
        error
      } = await supabase.from('grade_boundaries').insert({
        grade: formData.grade,
        min_score: minScore,
        max_score: maxScore
      });
  ...
  const handleDelete = async (boundary: GradeBoundary) => {
    ...
    const {
      error
    } = await supabase.from('grade_boundaries').delete().eq('id', boundary.id);
    if (error) {
      toast({
        title: 'Error',
  ...
  return <div>
      ...
      {loading ? <div className="text-center py-8">Loading...</div> : boundaries.length === 0 ? <div className="text-center py-8 text-muted-foreground">
            ...
            {boundaries.map(boundary => <TableRow key={boundary.id}>
                ...
                <TableCell className="text-right">
                  <div className="flex justify-end gap-2">
                    <Button variant="ghost" size="sm" onClick={() => handleEdit(boundary)}>
                      <Pencil className="w-4 h-4" />
                    </Button>
                    <Button variant="ghost" size="sm" onClick={() => handleDelete(boundary)}>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              {editingBoundary ? 'Update the score range for this grade' : 'Define the score range for a new grade'}
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4">
            ...
            })} placeholder="e.g., A, B, C" className="mt-1" disabled={!!editingBoundary} />
            </div>

            <div>
              <Label htmlFor="min_score">Minimum Score</Label>
              <Input id="min_score" type="number" value={formData.min_score} onChange={e => setFormData({
              ...formData,
              min_score: e.target.value
            })} placeholder="e.g., 90" min="0" max="100" className="mt-1" />
            </div>

            <div>
              <Label htmlFor="max_score">Maximum Score</Label>
              <Input id="max_score" type="number" value={formData.max_score} onChange={e => setFormData({
              ...formData,
              max_score: e.target.value
            })} placeholder="e.g., 100" min="0" max="100" className="mt-1" />
            </div>
          </div>
