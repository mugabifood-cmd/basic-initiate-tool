...
export default function TeacherManagement() {
  ...
  const fetchTeachers = async () => {
    try {
      ...
      const {
        data: teachersData,
        error: teachersError
      } = await supabase.from('profiles').select('*').eq('role', 'teacher');
      if (teachersError) throw teachersError;

      // Fetch assignments for each teacher
      const teachersWithAssignments = await Promise.all(teachersData.map(async teacher => {
        const {
          data: assignments,
  ...
  const fetchSubjects = async () => {
    const {
      data,
      error
    } = await supabase.from('subjects').select('id, name, code').order('name');
    if (error) {
  ...
  const fetchClasses = async () => {
    const {
      data,
      error
    } = await supabase.from('classes').select('id, name, stream').order('name');
    if (error) {
  ...
  const openEditDialog = (teacher: TeacherWithAssignments) => {
    ...
    teacher.assignments.forEach(assignment => {
      if (assignment.assignment_type === 'subject_teacher' && assignment.subject_id) {
        ...
        const emptySlotIndex = subjectGroups[assignment.subject_id].classSlots.findIndex((slot: ClassSlot) => !slot.className);
        if (emptySlotIndex !== -1) {
                ...
                return <TableRow key={teacher.id}>
                      ...
                      <TableCell>
                        <div className="flex flex-wrap gap-1">
                          {subjectAssignments.map((assignment, index) => <Badge key={index} variant="secondary" className="text-xs">
                              {assignment.subject?.name} - {assignment.class_name} {assignment.stream}
                            </Badge>)}
                          {subjectAssignments.length === 0 && <span className="text-muted-foreground text-sm">No subject assignments</span>}
                        </div>
                      </TableCell>
                      <TableCell>
                        {classAssignment ? <Badge variant="outline">
                            {classAssignment.class_name} {classAssignment.stream}
                          </Badge> : <span className="text-muted-foreground text-sm">No class assignment</span>}
                      </TableCell>
                      <TableCell>
                        <div className="flex space-x-2">
                          <Button variant="default" size="sm" onClick={() => openEditDialog(teacher)}>
                            Assign Subjects
                          </Button>
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                <Button type="button" variant="outline" size="sm" onClick={addSubjectAssignment}>
                  <Plus className="h-4 w-4 mr-2" />
                  Add Subject
                </Button>
              </div>
              
              {subjectAssignments.map((assignment, index) => <div key={index} className="border rounded-lg p-4 space-y-3">
                  <div className="flex items-center justify-between">
                    <Select value={assignment.subjectId} onValueChange={value => updateSubjectAssignment(index, value)}>
                    ...
                    <Button type="button" variant="outline" size="sm" onClick={() => removeSubjectAssignment(index)}>
                      <Trash2 className="h-4 w-4" />
                    </Button>
                  </div>
                  ...
                  <div>
                    ...
                    const availableStreamsForClass = selectedClass ? [...new Set(classes.filter(cls => cls.name === selectedClass.name).map(cls => cls.stream))] : [];

                    // Filter out streams that are already used for this subject
                    const availableStreams = availableStreamsForClass.filter(stream => !usedClasses.has(`${slot.className}-${stream}`) || stream === slot.stream);
                    return <div key={slotIndex} className="grid grid-cols-2 gap-3">
                            <div>
                              <Label htmlFor={`class-${index}-${slotIndex}`} className="text-xs text-gray-600">
                                Class {slotIndex + 1}
                              </Label>
                              <Select value={slot.className || "none"} onValueChange={value => updateClassSlot(index, slotIndex, 'className', value === 'none' ? '' : value)}>
                                <SelectTrigger>
                                  <SelectValue placeholder="Select class" />
                                </SelectTrigger>
                                <SelectContent>
                                  <SelectItem value="none">None</SelectItem>
                                  {uniqueClassNames.map(className => <SelectItem key={className} value={className}>
                                      {className}
                                    </SelectItem>)}
                                </SelectContent>
                              </Select>
                            ...
                            <div>
                              ...
                              <Select value={slot.stream || "none"} onValueChange={value => updateClassSlot(index, slotIndex, 'stream', value === 'none' ? '' : value)} disabled={!slot.className}>
                                <SelectTrigger>
                                  <SelectValue placeholder="Select stream" />
                                </SelectTrigger>
                                <SelectContent>
                                  <SelectItem value="none">None</SelectItem>
                                  {availableStreams.map(stream => <SelectItem key={stream} value={stream}>
                                      {stream}
                                    </SelectItem>)}
                                </SelectContent>
                              </Select>
              ...
              {subjectAssignments.length === 0 && <p className="text-sm text-muted-foreground">No subject assignments added yet.</p>}
            </div>
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <Select value={classAssignment?.className || 'none'} onValueChange={value => setClassAssignment(value !== 'none' ? {
                  ...classAssignment,
                  className: value,
                  stream: classAssignment?.stream || ''
                } : null)}>
                    ...
                    <SelectContent>
                      <SelectItem value="none">None</SelectItem>
                      {[...new Set(classes.map(cls => cls.name))].map(className => <SelectItem key={className} value={className}>
                          {className}
                        </SelectItem>)}
                    </SelectContent>
                  </Select>
                ...
                <div>
                  ...
                  <Select value={classAssignment?.stream || 'none'} onValueChange={value => setClassAssignment(classAssignment && value !== 'none' ? {
                  ...classAssignment,
                  stream: value
                } : null)} disabled={!classAssignment?.className}>
                    <SelectTrigger>
                      <SelectValue placeholder="Select stream" />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="none">None</SelectItem>
                      {classAssignment?.className && [...new Set(classes.filter(cls => cls.name === classAssignment.className).map(cls => cls.stream))].map(stream => <SelectItem key={stream} value={stream}>
                            {stream}
                          </SelectItem>)}
                    </SelectContent>
                  </Select>
